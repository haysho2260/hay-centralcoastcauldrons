    ...
    try:
        for potion in potions_delivered:
            sql = sqlalchemy.text(
                ...
                """
            ).bindparams(quantity=potion.quantity, potion_type=str(potion.potion_type))
...
def get_bottle_plan():
    ...
    with db.engine.begin() as connection:
        red, green = connection.execute(sqlalchemy.text(f"SELECT num_red_ml, num_green_ml FROM global_inventory")).first()
